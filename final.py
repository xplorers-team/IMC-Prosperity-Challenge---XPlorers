...
# Dictionary of all the products with initial positions set to 0
product_dict = {'AMETHYSTS': 0, 'STARFRUIT': 0}
...
# Main trading class
class Trader:
    position = copy.deepcopy(product_dict)  # Current position for each product
    POSITION_LIMIT = {'AMETHYSTS': 20, 'STARFRUIT': 20}  # Position limit for each product
    volume_traded = copy.deepcopy(product_dict)  # Volume traded for each product
    cpnl = defaultdict(lambda: 0)  # Dictionary to track cumulative profit and loss for each product
    amethysts_cache = []  # Cache for amethysts prices (stores mean prices for past dim no. of days)
    STARFRUIT_cache = []  # Cache for starfruit prices
    amethysts_dim = 5  # Dimension of amethysts cache (cache: storing previous day prices)
    STARFRUIT_dim = 5  # Dimension of starfruit cache
    steps = 0  # Number of steps in the trading process
    ...
    # ema_prices keeps an exponential moving average of prices
    ema_prices = dict()
    for product in PRODUCTS:
