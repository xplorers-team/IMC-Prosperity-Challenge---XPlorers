...
class Trader:
    ...
    def black_scholes(self, state):
        ...
        if pd.isna(rolling_volatility):
            fair_call = self.black_scholes_price(mid_price, strike, T, rate, rolling_volatility)
        else:
        ...
        rolling_20_spread = self.prices["SPREAD_COCO"].rolling(20).mean().iloc[-1]

        if best_ask < fair_call - 5: 
